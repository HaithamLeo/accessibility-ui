...
const ToolsPanel: FC<ToolsPanelProps> = ({ widgetState, onChangeWidgetState, config }) => {
  return (
    <>
      {(config?.bigCursor !== false) && <BigCursorButton widgetState={widgetState} onChangeWidgetState={onChangeWidgetState} />}
      {(config?.readingGuide !== false) && <ReadingGuide widgetState={widgetState} onChangeWidgetState={onChangeWidgetState} />}
    </>
  )
}
